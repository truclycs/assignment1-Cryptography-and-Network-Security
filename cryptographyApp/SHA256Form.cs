    ...
    public partial class SHA256Form : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void btnHash_Click(object sender, EventArgs e)
        {
            try
            {
                if (this._isText)
                {
                    textOutput.Text = cryptographyApp.hashFunction.hash(textInput.Text, SHA256.Create());
                }
                else
                {
                    textOutput.Text = cryptographyApp.hashFunction.hash(Utility.readFile(textInput.Text), SHA256.Create());
                }
            }
            catch (Exception ex)
            {
